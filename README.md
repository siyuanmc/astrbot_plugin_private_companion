...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.3.0 继续完善公开空间互动、主动生图/每日穿搭、关系网身份隔离和配置管理体验；默认行为保持稳，公开回复和主动图片能力仍建议按需开启。
...
- 默认关闭，避免和独立生图插件抢触发；开启后只在主人私聊里响应明确请求。
- 文生图示例：`帮我画一只趴在键盘旁边的白猫`、`生成一张雨夜街角插画`。
- 改图示例：随消息带图或引用近期图片后说 `改成蓝色渐变风格`、`把这张图加上绿色对钩牌子`。
